...
class BlogorderController extends BaseController{
   ...
   public function media_userinfo(){
        ...
        $mediafield .= "w.*";
        $media_userinfo = $this->mediaProcess->getMediaBloginfo($id,$mediafield);
        if($media_userinfo){
           ...
           $media_userinfo['postprice_adver'] = priceConversion($media_userinfo['postprice_adver']);
           $media_userinfo['forwardprice_adver'] = priceConversion($media_userinfo['forwardprice_adver']);
	...
	public function blogorder(){
        ...
        $tasks = M('task');
	    foreach($data as &$d){
       	      $info = $media_user -> field("name")-> where("id='{$d['payeeid']}'") ->find();
	    ...
	    $this -> assign('funcdesc',"微博帐号订单");
	    $this -> display();
	}

    //微博帐号后台提交申诉理由   或者 修改广告主和自媒体主的真实结算价
